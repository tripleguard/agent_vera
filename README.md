...
<<<<<<< HEAD
**Чтобы прервать разговор Веры** (например, когда она говорит длинный ответ), скажите **"Вера"** — она остановится и будет ждать новую команду.

Скачать агента: https://agentvera.ru
=======
Вы можете скачать агента на сайте https://agentvera.ru
>>>>>>> 9bc446f488e4e539c078019171cb601d2de41a12
...
| Приложения | Запуск, закрытие, запланированный запуск по расписанию |
| Окна | Управление окнами (свернуть, развернуть, переключить) |
| Файлы | Поиск и открытие файлов и папок с указанием диска |
...
| Инструменты | Чтение документов, интерпретатор Python-кода |
| Telegram | Отправка сообщений, чтение переписок, проверка новых сообщений |
| Стабильность | Автоперезапуск при ошибках (до 5 попыток) |
...
Установите [Microsoft Visual C++ Redistributable](https://aka.ms/vc14/vc_redist.x64.exe) затем:
...
**LLM:**
1. Скачайте модель GGUF (например Qwen3-4B-Q4_K_M)
...
Можно назначить автоматический запуск приложений по расписанию:
...
Вера, удали запуск телеги
```

**Варианты периодичности:**
- Ежедневно: "каждый день", "постоянно", "всегда", "регулярно"
- По будням: "по будням", "в рабочие дни"
- По выходным: "по выходным", "на выходных"
- Однократно: без указания периодичности

**Умное определение даты:** если время уже прошло сегодня, запуск планируется на завтра.
...
Вера, открой папку бокс на диске e
```

**Поиск с указанием диска:**
- Поддерживаются русские буквы
- Прямой поиск на указанном диске (до 3 уровней глубины)
- Fallback на Windows Search с фильтрацией по диску
...
<<<<<<< HEAD
Агент выполняет поиск через Brave, извлекает текст, затем формирует краткий ответ.

## Инструменты (Tools)

Агент имеет систему плагинов для расширенных возможностей.

### Чтение документов (read_document)

Поиск и чтение содержимого файлов:
...
**Поддерживаемые форматы:** `.txt`, `.md`, `.docx`, `.doc`, `.pdf`

Длинные документы автоматически суммаризируются через LLM.
...
**Особенности:**
- Таймаут 30 секунд (защита от бесконечных циклов)
- Код выполняется в отдельном процессе
- Только для локального использования (без песочницы)

## Telegram

Интеграция с Telegram через Telethon (требуется авторизация при первом использовании).

### Отправка сообщений

```
Вера, напиши Назару привет
Вера, отправь маме что буду поздно
```
...
Вера, прочти сообщения от мамы
```

**Умный статус:** если последнее сообщение твоё — покажет "пока не ответил".
...
**Особенности:**
- Автоматическая нормализация падежей имён ("Андрею" - "Андрей")
...
Скажите "Вера" — она остановится и будет ждать новую команду.
